...
export class NgxCarousel3dComponent implements OnChanges {
  ...
  buildCarousel(slides, options) {
    ...
    var carousel = this.$carouselService.build(this.slides || [], this.options || {})
      .then( (carousel) => {
  ...
  render(animate, speedTime) {
    ...
    // == Set the selected slide class name
    this.getSlide(this.carousel3d.selectedIndex).find('.slide-3d').addClass('selected')
  ...
  setCss(i, zIndex, positive) {
    const leftRemain = (this.carousel3d.space === 'auto') ? ((i + 1) * (this.carousel3d.width / 1.5)) : ((i + 1) * (this.carousel3d.space)),
      transform = (positive) ?
  ...
  goSlide(index) {
    ...
    this.changed.emit(this.slides[this.carousel3d.currentIndex]);
    ...
    $('.carousel-3d').find('.current').removeClass('current');
    $('.carousel-3d').find('.selected').removeClass('selected');
  ...
  slideClicked(index) {
    ...
    this.carousel3d.setSelectedIndex(index);
    if (this.carousel3d.currentIndex !== index) {
      ...
      // == Set the selected slide class name
      this.getSlide(this.carousel3d.selectedIndex).find('.slide-3d')
      .addClass('selected')
